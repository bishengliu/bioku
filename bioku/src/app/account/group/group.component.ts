...
export class GroupComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(private fb: FormBuilder, private alertService: AlertService, private route: ActivatedRoute,
    private updateGroupProfileService: UpdateGroupProfileService, @Inject(APP_CONFIG) private appSetting: any,
    @Inject(AppStore) private appStore, private refreshService: RefreshService, private router: Router,
  ...
  onUpdate(values: any): void {
    ...
    this.appStore.dispatch(
      updateGroupProfileActionAsync(
        formData, this.id, this.updateGroupProfileService, this.http, this.logAppStateService, this.alertService, this.refreshService));
    // naviagate to home
  ...
  ngOnInit() {
    this.sub = this.route.params.subscribe(params => {
       ...
       const that = this;
       // get the group
       const groupArray = this.groups.filter(function(d, i){
         return +d.pk === that.id;
       });
