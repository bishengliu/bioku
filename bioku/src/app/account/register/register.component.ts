...
export class RegisterComponent implements OnInit {
  ...
  constructor(fb: FormBuilder, private alertService: AlertService, @Inject(APP_CONFIG) private appSetting: any,
    @Inject(AppStore) private appStore, private registerService: RegisterService,
    ...
    this.registerForm = fb.group({
      'username': ['', Validators.compose([Validators.required, this.cValidators.usernameValidator()]),
        this.cValidators.usernameAsyncValidator()],
  ...
  onRegister(values: any): void {
    ...
    this.appStore.dispatch(registerActionAsync(formData, this.registerService,
      this.http, this.logAppStateService, this.alertService, this.refreshService));
  }

  updateState() {
    const state = this.appStore.getState();
    if (state.authInfo) {
      const isLogin = state.authInfo.token ? true : false;
      if (isLogin) {
