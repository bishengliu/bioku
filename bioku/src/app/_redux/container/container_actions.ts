...
(dispatch: Dispatch<AppState>, getState) => {
    ...
    .subscribe(
        (data: Container) => {
            const setCurrentContainerAction: SetCurrentContainerAction = setCurrentContainerActionCreator(data);
...
(dispatch: Dispatch<AppState>, getState) => {
    ...
    .subscribe(
        data => {
            ...
            const setMyContainersAction: SetMyContainersAction = setMyContainersActionCreator(data);
            dispatch(setMyContainersAction);
            // dumpdata to the locastorage
            refreshService.dumpContainerState(getState().containerInfo);
            // get state: apppartialstate
            const nextState: AppPartialState = logAppStateService.getAppPartialState();
            const message = 'query my containers success!';
...
(dispatch: Dispatch<AppState>, getState) => {
    ...
    .subscribe( data => {
            ...
            const preState: AppPartialState = logAppStateService.getAppPartialState();
            // set current box
            const setCurrentBoxAction: SetCurrentBoxAction = setCurrentBoxActionCreator(container, box);
            dispatch(setCurrentBoxAction);

            // get state: apppartialstate
            const nextState: AppPartialState = logAppStateService.getAppPartialState();
            const message = 'set current box!';
            // logger the redux action
