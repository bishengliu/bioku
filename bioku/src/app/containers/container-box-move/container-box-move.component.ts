...
export class ContainerBoxMoveComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.boxes.length > 0) {
      this.boxes.forEach((box, i) => {
        const move_box: MoveBox = new MoveBox();
  ...
  selectContainer(box_full_position: string, event: any) {
    ...
    const all_box_positions: Array<string> = this.obtainBoxPostions(this.boxes);
    // get the index
    const box_index: number = all_box_positions.indexOf(box_full_position);
    if (box_index !== -1) {
  ...
  genOptions(box_full_position: string, type: string): Array<number> {
    ...
    const target_container = this.move_boxes.filter((m, i) => {
  ...
  save_move_box() {
    ...
    const filteredMoveBoxes = this.filterMoveBoxes(this.move_boxes);
    if (filteredMoveBoxes.length > 0) {
      let failed_boxes: string = null;
      let count = 0;
      filteredMoveBoxes.forEach((mb, i) => {
        count++;
        this.containerService.moveContainerBoxes(mb.original_container,
                                                 mb.box_full_position, mb.target_container, mb.target_box_full_position)
        .subscribe(() => {
  ...
  filterMoveBoxes(move_boxes: Array<MoveBox>) {
    if (move_boxes.length > 0) {
      ...
      const step1: Array<MoveBox> = move_boxes.filter((box, i) => {
        return ((box.is_excluded == null || box.is_excluded === false)
                && (box.target_tower != null && box.target_shelf != null && box.target_box != null));
      });
      if (step1.length > 0) {
