...
import { setCurrentContainerActionAsync, SetCurrentBoxAction,
        setCurrentBoxActionCreator, SetCurrentContainerAction,
        setCurrentContainerActionCreator } from '../../_redux/container/container_actions';
@Component({
...
export class ContainerBoxOverviewActionPanelComponent implements OnInit, OnChanges {
  ...
  retrieveBox(boxAvailability: BoxAvailability) {
    if (this.all_boxes != null) {
      const boxes = this.all_boxes.filter((b, i) => { return b.box_position === boxAvailability.full_position});
      if (boxes.length > 0) {
  ...
  displaySelectedBox(box: Box): void {
    const setCurrentBoxAction: SetCurrentBoxAction = setCurrentBoxActionCreator(this._container, box);
  ...
  ngOnChanges(change: SimpleChanges) {
    ...
    if (change['lastSelectedOccupiedBox'] !== undefined) {
      if (this.lastSelectedOccupiedBox != null) {
        ...
        .subscribe((box: Box) => {
          this.selected_single_occupied_box = box;
        }, () => {
  ...
  switchBox() {
    ...
    if (this.selectedOccupiedSlots.length === 2) {
      ...
      const swtich_box: MoveBox = new MoveBox();
      ...
      this.containerService.moveContainerBoxes(swtich_box.original_container, swtich_box.box_full_position,
                                              swtich_box.target_container, swtich_box.target_box_full_position)
      .subscribe(() => {
